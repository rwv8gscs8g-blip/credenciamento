...
If Not CR_ExecutarCredenciamentoLote(empId, cnpj, razao, ativId, ativDesc, CR_Lista.ListIndex, _
                                      True, True, totalServ, adicionados, ignorados, _
                                      faltantes, msgSave, detalheErro, tempoSeg) Then
    Select Case detalheErro
        Case "SEM_SERVICOS"
        ...
        Case Else
            MsgBox "Erro ao credenciar empresa: " & detalheErro, vbCritical, "Credenciamento"
    End Select
    Exit Sub
End If
...
Private Function CR_ExecutarCredenciamentoLote( _
    ...
    ByVal atualizarMenu As Boolean, _
    ByRef totalServ As Long, _
    ByRef adicionados As Long, _
    ByRef ignorados As Long, _
    ByRef faltantes As String, _
    ByRef msgSave As String, _
    ByRef detalheErro As String, _
    ByRef tempoSeg As Double _
) As Boolean
    ...
    Dim posNova As Long
    Dim proximaPos As Long
    Dim proximoCredSeq As Long
    Dim maxCredReal As Long
    ...
    Dim indiceCred As Object
    Dim inicioTimer As Double
    Dim chaveCred As String
    ...
    If Not Util_PrepararAbaParaEscrita(wsCred, estavaProtegida, senhaProtecao) Then
        detalheErro = "ABA_PROTEGIDA"
        GoTo finalizar
    End If
    abaPreparada = True

    ' Onda 38.2.2 AT-5: bloco rapido envelopa o loop de credenciamento.
    ...
    Set indiceCred = CR_CriarIndiceCredenciamentos(wsCred, empId)
    ultimaServ = UltimaLinhaAba(SHEET_CAD_SERV)
    ultimaCred = UltimaLinhaAba(SHEET_CREDENCIADOS)
    If ultimaCred < LINHA_DADOS Then ultimaCred = LINHA_DADOS - 1
    proximaPos = ProximaPosicaoAtividade(wsCred, ativId)
    proximoCredSeq = CLng(Val(wsCred.Cells(1, COL_CONTADOR_AR).Value))
    maxCredReal = Util_MaxIdOperacional(SHEET_CREDENCIADOS)
    If maxCredReal > proximoCredSeq Then proximoCredSeq = maxCredReal

    ' FT-4: aloca CRED_ID/linha/posicao em lote; nao chama ProximoId dentro do loop.
    For i = LINHA_DADOS To ultimaServ
        If IdsIguaisCred(wsServ.Cells(i, COL_SERV_ATIV_ID).Value, ativId) Then
            ...
            chaveCred = CR_ChaveCred(codAtivServ, empId)

            If Not CR_CredJaExisteRapido(wsCred, indiceCred, chaveCred, codAtivServ, empId) Then
                ultimaCred = ultimaCred + 1
                linhaNova = ultimaCred
                posNova = proximaPos
                proximaPos = proximaPos + 1
                proximoCredSeq = proximoCredSeq + 1
                credId = Format$(proximoCredSeq, "000")
                ...
                CR_AdicionarIndiceCred indiceCred, chaveCred
                If ATIVAR_DIAG_FNEW5 Then
    ...
    If adicionados > 0 Then wsCred.Cells(1, COL_CONTADOR_AR).Value = proximoCredSeq

    If totalServ = 0 Then
        detalheErro = "SEM_SERVICOS"
        GoTo finalizar
    End If

    Call ClassificaCredenciadoOrdem
    If atualizarMenu Then Call AtualizarListaEmpresaMenuAtual
    If ATIVAR_DIAG_FNEW5 Then
        DIAG_FNEW5_RegistrarCredenciamentos diagCsvPath, diagCreds, wsCred, empId, cnpj, razao, _
            ativId, ativDesc, totalServ, adicionados, ignorados, listIndex, _
            "POS_RELOAD_PRE_VALIDACAO", ""
    End If

    If Not ValidarPersistenciaCredenciamento(wsCred, wsServ, empId, ativId, faltantes) Then
        detalheErro = "FALHA_PERSISTENCIA"
        GoTo finalizar
    End If
    ...
    If salvarWorkbook Then
        If Not Util_SalvarWorkbookSeguro(msgSave) Then
            ' O caller exibe o alerta, mas credenciamento e persistencia ja passaram.
        End If
    End If

    CR_ExecutarCredenciamentoLote = True

finalizar:
    On Error Resume Next
    If abaPreparada Then Call Util_RestaurarProtecaoAba(wsCred, estavaProtegida, senhaProtecao)
    If blocoRapidoIniciado Then Util_FinalizarBlocoRapido estadoExcel
    tempoSeg = Timer - inicioTimer
    If tempoSeg < 0# Then tempoSeg = tempoSeg + 86400#
    On Error GoTo 0
    Exit Function

falha:
    detalheErro = Err.Description
    CR_ExecutarCredenciamentoLote = False
    Resume finalizar
End Function

Public Function TV2_ExecutarCredenciamentoLote( _
    ByVal empId As String, _
    ByVal ativId As String, _
    ...
    Dim msgSave As String
    Dim detalheErro As String
    Dim empCanon As String
    Dim ativCanon As String

    empCanon = Pad3(empId)
    ativCanon = Pad3(ativId)
    If empCanon = "" Or ativCanon = "" Then
        detalhes = "EMP_ID ou ATIV_ID vazio."
        Exit Function
    End If

    If Not CarregarDadosEmpresaSelecionada(empCanon, cnpj, razao) Then
        detalhes = "Empresa nao encontrada: " & empCanon
        Exit Function
    End If

    TV2_ExecutarCredenciamentoLote = CR_ExecutarCredenciamentoLote( _
        empCanon, cnpj, razao, ativCanon, "TV2 FT4", -1, _
        False, False, totalServOut, adicionadosOut, ignoradosOut, _
        faltantes, msgSave, detalheErro, tempoSegOut)

    detalhes = "TOTAL_SERV=" & CStr(totalServOut) & _
               "; ADICIONADOS=" & CStr(adicionadosOut) & _
               "; IGNORADOS=" & CStr(ignoradosOut) & _
               "; FALTANTES=" & IIf(faltantes = "", "nenhum", faltantes) & _
               "; ERRO=" & IIf(detalheErro = "", "nenhum", detalheErro) & _
               "; TEMPO_SEG=" & Format$(tempoSegOut, "0.000")
End Function

Private Function CR_CriarIndiceCredenciamentos(ByVal wsCred As Worksheet, ByVal empId As String) As Object
    Dim dic As Object
    Dim i As Long
    Dim chave As String
    ...
    For i = LINHA_DADOS To UltimaLinhaAba(SHEET_CREDENCIADOS)
        If IdsIguaisCred(wsCred.Cells(i, COL_CRED_EMP_ID).Value, empId) Then
            chave = CR_ChaveCred(wsCred.Cells(i, COL_CRED_COD_ATIV_SERV).Value, empId)
            If chave <> "" Then
                If Not dic.Exists(chave) Then dic.Add chave, True
            End If
        End If
    Next i

    Set CR_CriarIndiceCredenciamentos = dic
    Exit Function

falha:
    Set CR_CriarIndiceCredenciamentos = Nothing
End Function

Private Function CR_CredJaExisteRapido( _
    ByVal wsCred As Worksheet, _
    ByVal indiceCred As Object, _
    ByVal chaveCred As String, _
    ByVal codAtivServ As String, _
    ByVal empId As String _
) As Boolean
    If Not indiceCred Is Nothing Then
        CR_CredJaExisteRapido = indiceCred.Exists(chaveCred)
    Else
        CR_CredJaExisteRapido = CredJaExiste(wsCred, codAtivServ, empId)
    End If
End Function

Private Sub CR_AdicionarIndiceCred(ByVal indiceCred As Object, ByVal chaveCred As String)
    On Error Resume Next
    If Not indiceCred Is Nothing Then
        If chaveCred <> "" Then
            If Not indiceCred.Exists(chaveCred) Then indiceCred.Add chaveCred, True
        End If
    End If
    On Error GoTo 0
End Sub

Private Function CR_ChaveCred(ByVal codAtivServ As Variant, ByVal empId As Variant) As String
    CR_ChaveCred = NormalizarCodAtivServ(codAtivServ) & "|" & Pad3(empId)
End Function
