...
Private Sub SV_CR_Lista_Click()
    ...
    Err.Clear
    On Error GoTo erro_carregamento
    Call Rel_FormatarCabecalho(wsRel, 4)
    Call Rel_FormatarDados(wsRel, LINHA_DADOS, linhaOut - 1, 4)
    Call Rel_ConfigurarPagina(wsRel, "RELATORIO DE EMPRESAS CREDENCIADAS POR SERVICO", "D", False, xlPortrait)
    wsRel.PageSetup.PrintArea = wsRel.Range("A1:D" & CStr(linhaOut - 1)).Address
