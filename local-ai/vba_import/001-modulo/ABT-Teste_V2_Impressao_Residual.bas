...
Public Sub TV2_RunImpressaoResidual(Optional ByVal visual As Boolean = False, Optional ByVal silencioso As Boolean = False)
    ...
    Dim detalhesQuantidade As String
    Dim erroFatalNumero As Long
    Dim erroFatalDescricao As String

    On Error GoTo falha

    TV2_InitExecucao TV2_IR_SUITE, visual, 7
    inicializado = True
    ...
    TV2_LogAssert TV2_IR_SUITE, "IR_05_AVALIACAO_BORDA_VERTICAL", "AUTO", _
                  "PreencherAvaliacaoOS reaplica borda externa esquerda da faixa vertical", _
                  "IMP_AVALIA!" & Preencher_RangeBordaAvaliacaoVertical() & " tem borda esquerda preta/continua/media", _
                  detalhesAvalBorda, _
                  "Fecha o marginal visual da faixa AVALIACAO em A25:A45 com peso externo medio", _
                  okAvalBorda
    ...
    okQuantidade = TV2_IR_QuantidadeInteiraOk(detalhesQuantidade)
    TV2_LogAssert TV2_IR_SUITE, "IR_07_QUANTIDADE_INTEIRA", "AUTO", _
                  "Quantidades impressas usam formato inteiro", _
                  "Campos Quant. exibem 1 sem separador decimal residual", _
                  detalhesQuantidade, _
                  "Fecha residual visual em que Quant. imprimia 1, por NumberFormatLocal 0,##", _
                  okQuantidade
...
Private Function TV2_IR_BordaAvaliacaoOk(ByRef detalhes As String) As Boolean
    ...
    Set ws = ThisWorkbook.Sheets("IMP_AVALIA")
    TV2_IR_BordaAvaliacaoOk = TV2_IR_BordaPretaContinua(ws.Range(Preencher_RangeBordaAvaliacaoVertical()).Borders(xlEdgeLeft), detalhes, xlMedium)
    Exit Function
...
Private Function TV2_IR_BordaPretaContinua(ByVal borda As Border, ByRef detalhes As String, Optional ByVal pesoEsperado As Long = 0) As Boolean
    Dim pesoOk As Boolean

    detalhes = "LineStyle=" & CStr(borda.LineStyle) & _
               "; Weight=" & CStr(borda.Weight) & _
               "; Color=" & CStr(borda.Color)
    pesoOk = (pesoEsperado = 0 Or borda.Weight = pesoEsperado)
    TV2_IR_BordaPretaContinua = (borda.LineStyle = xlContinuous And borda.Color = vbBlack And pesoOk)
End Function

Private Function TV2_IR_QuantidadeInteiraOk(ByRef detalhes As String) As Boolean
    Dim wsOS As Worksheet
    Dim wsPreOS As Worksheet
    Dim wsAval As Worksheet
    Dim detOS23 As String
    Dim detOS55 As String
    Dim detPre23 As String
    Dim detAval23 As String
    Dim okOS23 As Boolean
    Dim okOS55 As Boolean
    Dim okPre23 As Boolean
    Dim okAval23 As Boolean

    On Error GoTo falha

    Set wsOS = ThisWorkbook.Sheets("EMITE_OS")
    Set wsPreOS = ThisWorkbook.Sheets("EMITE_PREOS")
    Set wsAval = ThisWorkbook.Sheets("IMP_AVALIA")

    okOS23 = TV2_IR_QuantidadeCelulaInteiraOk(wsOS.Range("L23"), detOS23)
    okOS55 = TV2_IR_QuantidadeCelulaInteiraOk(wsOS.Range("L55"), detOS55)
    okPre23 = TV2_IR_QuantidadeCelulaInteiraOk(wsPreOS.Range("L23"), detPre23)
    okAval23 = TV2_IR_QuantidadeCelulaInteiraOk(wsAval.Range("L23"), detAval23)

    detalhes = "EMITE_OS_L23={" & detOS23 & "}; " & _
               "EMITE_OS_L55={" & detOS55 & "}; " & _
               "EMITE_PREOS_L23={" & detPre23 & "}; " & _
               "IMP_AVALIA_L23={" & detAval23 & "}"
    TV2_IR_QuantidadeInteiraOk = (okOS23 And okOS55 And okPre23 And okAval23)
    Exit Function
...
Private Function TV2_IR_QuantidadeCelulaInteiraOk(ByVal alvo As Range, ByRef detalhes As String) As Boolean
    Dim valor As Double
    Dim texto As String
    Dim formato As String
    Dim formatoLocal As String

    On Error GoTo falha

    valor = Util_Conversao.ToDouble(CStr(alvo.Value))
    texto = SafeListVal(alvo.Text)
    formato = SafeListVal(alvo.NumberFormat)
    formatoLocal = SafeListVal(alvo.NumberFormatLocal)
    detalhes = "Value=" & CStr(valor) & _
               "; Text=" & texto & _
               "; NumberFormat=" & formato & _
               "; NumberFormatLocal=" & formatoLocal
    TV2_IR_QuantidadeCelulaInteiraOk = _
        (Abs(valor - 1#) < 0.0001 And _
         formato = "0" And _
         formatoLocal = "0" And _
         InStr(1, texto, ",", vbTextCompare) = 0)
    Exit Function
