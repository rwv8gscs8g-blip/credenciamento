...
Public Sub TV2_GerarCatalogoBase()
    ...
    ' === Onda 7 (V12.0.0203) - Familias IDM_* (idempotencia administrativa) + RDZ_* ===
    TV2_AddCatalogo ws, nr, "IDM_001", "IDEMPOTENCIA", "RAPIDO", "AUTO", "Limpa_Base", "Limpa_Base 3x consecutivas = mesmo estado final", "Base canonica limpa; chamada Limpa_Base sem alteracao entre execucoes", "Validar idempotencia administrativa de Limpa_Base", "Estado apos 1a, 2a e 3a execucao identico (mesmas contagens nas abas)", "Garante que execucao repetida nao introduz lixo nem remove dados validos", "AUTOMATIZADO_0203", "Executado na bateria oficial"
    TV2_AddCatalogo ws, nr, "IDM_002", "IDEMPOTENCIA", "RAPIDO", "AUTO", "Reset_CNAE", "Reset_CNAE 2x = mesma quantidade de atividades", "Atividades canonicas carregadas; chamar ResetarECarregarCNAE_Padrao 2x", "Validar idempotencia do reset de CNAE", "1a execucao carrega N atividades; 2a execucao mantem N (zero duplicatas, zero remocoes)", "Pega bug Empresa-zumbi tipo cabecalho corrompido na lista CNAE", "AUTOMATIZADO_0203", "Executado na bateria oficial"
    TV2_AddCatalogo ws, nr, "IDM_003", "IDEMPOTENCIA", "COMPLETO", "AUTO", "Limpa_Base", "Cabecalho corrompido em EMPRESAS - Limpa_Base restaura canonico", "EMPRESAS com cabecalho linha 1 sobrescrito por valor invalido", "Validar restauracao automatica do cabecalho canonico", "Apos Limpa_Base: cabecalho linha 1 igual ao canonico; dados zerados (so cabecalho)", "Cobertura direta do bug que motivou criacao da familia (Onda 5 retro)", "AUTOMATIZADO_0203", "Executado na bateria oficial"
    TV2_AddCatalogo ws, nr, "IDM_004", "IDEMPOTENCIA", "COMPLETO", "AUTO", "Limpa_Base", "UsedRange vazado - Limpa_Base recoloca area real", "Forcar UsedRange de EMPRESAS para 1 048 576 linhas via formatacao residual", "Validar recolocacao do UsedRange para area de dados real", "Apos Limpa_Base: UsedRange.Rows.Count <= cabecalho + margem (1 ou 2 linhas)", "Evita planilha incrivelmente lenta por UsedRange inflado", "AUTOMATIZADO_0203", "Executado na bateria oficial"
    TV2_AddCatalogo ws, nr, "RDZ_001", "RODIZIO", "COMPLETO", "AUTO", "Rodizio", "3 emissoes sucessivas + 4a com bloqueio + retomada apos conclusao", "Base canonica limpa com 3 empresas no item; ciclo completo de emissao + OS + avaliacao", "Validar contrato sequencial do rodizio em 4 ciclos com 1 retomada", "1a -> EMP_001; 2a -> EMP_002; 3a -> EMP_003; 4a -> SEM_CREDENCIADOS_APTOS (todos com OS); apos concluir EMP_001 -> nova emissao retorna a EMP_001", "Prova ponta a ponta o ciclo + bloqueio + retomada do rodizio canonico", "AUTOMATIZADO_0203", "Executado na bateria oficial"
...
Private Sub TV2_GerarRoteiroAssistido()
    ...
    TV2_AddRoteiro ws, nr, "IDM_001", "AUTO", "Validar idempotencia de Limpa_Base", "Executar a bateria oficial e conferir a linha IDM_001", "Estado identico apos 3 execucoes consecutivas (mesmas contagens em CREDENCIADOS, EMPRESAS, ATIVIDADES, CAD_SERV)", "Linha do cenario IDM_001", "Detecta acumulo de lixo ou remocao indevida em execucoes repetidas", "AUTOMATIZADO"
    TV2_AddRoteiro ws, nr, "IDM_002", "AUTO", "Validar idempotencia de Reset_CNAE", "Executar a bateria oficial e conferir a linha IDM_002", "Mesma quantidade de atividades apos 1a e 2a execucao; zero duplicatas; zero remocoes na 2a", "Linha do cenario IDM_002", "Detecta drift na lista canonica de CNAE", "AUTOMATIZADO"
    TV2_AddRoteiro ws, nr, "IDM_003", "AUTO", "Validar restauracao de cabecalho corrompido", "Executar a bateria oficial e conferir a linha IDM_003", "Cabecalho linha 1 igual ao canonico apos Limpa_Base; dados zerados", "Linha do cenario IDM_003", "Pega bug Empresa-zumbi tipo cabecalho corrompido", "AUTOMATIZADO"
    TV2_AddRoteiro ws, nr, "IDM_004", "AUTO", "Validar recolocacao de UsedRange vazado", "Executar a bateria oficial e conferir a linha IDM_004", "UsedRange.Rows.Count <= 2 apos Limpa_Base (cabecalho + margem)", "Linha do cenario IDM_004", "Evita planilha lenta por UsedRange inflado", "AUTOMATIZADO"
    TV2_AddRoteiro ws, nr, "RDZ_001", "AUTO", "Validar ciclo + bloqueio + retomada do rodizio", "Executar a bateria oficial e conferir a linha RDZ_001", "Sequencia: 1a->001; 2a->002; 3a->003; 4a->SEM_CREDENCIADOS_APTOS; apos concluir 001 -> nova->001", "Linha do cenario RDZ_001", "Prova ponta a ponta o ciclo de rodizio canonico", "AUTOMATIZADO"
