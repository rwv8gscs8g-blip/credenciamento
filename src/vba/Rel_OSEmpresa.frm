...
Private Sub B_RelEmpresaOS_Click()
    Call AcionarRelatorioOSEmpresa
End Sub

Private Sub B_RelMEIOS_Click()
    Call AcionarRelatorioOSEmpresa
End Sub

Private Sub AcionarRelatorioOSEmpresa()
...
Private Sub GerarImprimirRelatorioOSEmpresa()
    ...
    Dim Var5 As String, Var6 As String, Var7 As String
    Dim Var8 As Variant
    ...
    For linhaAtual = LINHA_DADOS To ultimaOS
        If IdsIguais(wsOS.Cells(linhaAtual, COL_OS_EMP_ID).Value, empresaId) Then
            ...
            If incluirLinha Then
                ...
                Var8 = wsOS.Cells(linhaAtual, COL_OS_NOTA).Value
                ...
                If Trim$(SafeListVal(Var8)) <> "" Then
                    wsRel.Cells(relLinha, 8).Value = Util_Conversao.ToDouble(Var8)
                Else
                    wsRel.Cells(relLinha, 8).ClearContents
                End If
                relLinha = relLinha + 1
            End If
        End If
    ...
    wsRel.Range("H2:H" & CStr(relLinha - 1)).NumberFormat = "0.00"
    wsRel.Range("H2:H" & CStr(relLinha - 1)).HorizontalAlignment = xlRight
