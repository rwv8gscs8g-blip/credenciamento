...
Private Sub AV_AplicarDemandanteImpressao(ByVal demandanteNome As String)
    Desc_entidade = demandanteNome
End Sub

Public Function TV2_AvaliacaoDemandanteNaLista( _
    ...
    For i = 0 To AV_Lista.ListCount - 1
        If IdsIguais(SafeListVal(AV_Lista.List(i, 0)), osId) Then
            demandanteOut = Trim$(SafeListVal(AV_Lista.List(i, 1)))
            detalhesOut = "OS_ID=" & osId & "; LINHA_LISTA=" & CStr(i) & _
                          "; DEMANDANTE_LISTA=" & demandanteOut
            TV2_AvaliacaoDemandanteNaLista = (demandanteOut <> "")
            Exit Function
        End If
    Next i

    detalhesOut = "OS_ID=" & osId & "; nao encontrado em AV_Lista"
    TV2_AvaliacaoDemandanteNaLista = False
    Exit Function

falha:
    detalhesOut = "Erro " & CStr(Err.Number) & ": " & Err.Description
    TV2_AvaliacaoDemandanteNaLista = False
End Function

Public Function TV2_AvaliacaoAplicarDemandanteImpressao( _
    ByVal osId As String, _
    ByRef demandanteOut As String, _
    ByRef detalhesOut As String _
) As Boolean
    ...
    demandanteOut = ""
    detalhesOut = ""
    res = ResolverDemandanteAvaliacaoPorOS(osId, entId, nome, detalhesSvc)
    If Not res.sucesso Then
        detalhesOut = res.mensagem & "; " & detalhesSvc
        TV2_AvaliacaoAplicarDemandanteImpressao = False
        Exit Function
    End If

    Desc_entidade = "__TV2_OBSOLETO__"
    Call AV_AplicarDemandanteImpressao(nome)
    demandanteOut = Trim$(SafeListVal(Desc_entidade))
    detalhesOut = detalhesSvc & "; DESC_ENTIDADE=" & demandanteOut
    TV2_AvaliacaoAplicarDemandanteImpressao = (demandanteOut = nome)
    Exit Function
...
Private Sub BE_ImprimeOS_Click()
    ...
    NR_Empenho = SafeListVal(N_Empenho.Value)
    ...
    For idxEnt = LINHA_DADOS To UltimaLinhaAba(SHEET_ENTIDADE)
        If IdsIguais(SafeListVal(wsEnt.Cells(idxEnt, COL_ENT_ID).Value), OSListaCol(6)) Then
            ...
            END_ENTIDADE = SafeListVal(wsEnt.Cells(idxEnt, COL_ENT_ENDERECO).Value)
            Exit For
        End If
    Next idxEnt
...
Private Sub EncerraOS_Click()
    ...
    resDemandante = ResolverDemandanteAvaliacaoPorOS(osId, demandanteEntId, demandanteNome, demandanteDetalhes)
    If Not resDemandante.sucesso Then
        MsgBox "Não foi possível resolver o demandante da OS " & osId & "." & vbCrLf & _
               resDemandante.mensagem, vbExclamation, "Avaliação"
        GoTo limpar
    End If
    ...
    If MsgBox("Confirma a avaliação?" & vbCrLf & vbCrLf & _
              "OS: " & osId & vbCrLf & _
              "Demandante: " & demandanteNome & vbCrLf & _
    ...
    If MsgBox("Deseja imprimir a avaliação desta empresa?", vbQuestion + vbYesNo, "Avaliação") = vbYes Then
        N_OS = AVListaCol(0)
        Call AV_AplicarDemandanteImpressao(demandanteNome)
...
On Error GoTo erro_carregamento:
    ...
    idx = C_Lista.ListIndex
    C_Cad = CInt(Val(SafeListVal(C_Lista.List(idx, 0))))
    entidade = SafeListVal(C_Lista.List(idx, 0))
    Desc_entidade = SafeListVal(C_Lista.List(idx, 2))
    cont_entidade = SafeListVal(C_Lista.List(idx, 11))
    telcont_entidade = SafeListVal(C_Lista.List(idx, 12))
    END_ENTIDADE = SafeListVal(C_Lista.List(idx, 6))

    ' Onda 38.2.7: selecao deve ter paridade com o form de edicao.
    ' Colunas do C_Lista (0-based) = colunas da aba ENTIDADE (1-based) deslocadas em -1.
    C_CNPJ.Value = SafeListVal(C_Lista.List(idx, 1))
    C_Entidade.Value = SafeListVal(C_Lista.List(idx, 2))
    C_Tel_Fixo.Value = SafeListVal(C_Lista.List(idx, 3))
    C_Tel_Cel.Value = SafeListVal(C_Lista.List(idx, 4))
    C_Email.Value = SafeListVal(C_Lista.List(idx, 5))
    C_Endereco.Value = SafeListVal(C_Lista.List(idx, 6))
    C_Bairro.Value = SafeListVal(C_Lista.List(idx, 7))
    C_Municipio.Value = SafeListVal(C_Lista.List(idx, 8))
    C_CEP.Value = SafeListVal(C_Lista.List(idx, 9))
    C_UF.Value = SafeListVal(C_Lista.List(idx, 10))
    C_Contato1.Value = SafeListVal(C_Lista.List(idx, 11))
    C_Fone_Cont1.Value = SafeListVal(C_Lista.List(idx, 12))
    C_Func_Cont1.Value = SafeListVal(C_Lista.List(idx, 13))
    C_Contato2.Value = SafeListVal(C_Lista.List(idx, 14))
    C_Fone_Cont2.Value = SafeListVal(C_Lista.List(idx, 15))
    C_Func_Cont2.Value = SafeListVal(C_Lista.List(idx, 16))
    C_Contato3.Value = SafeListVal(C_Lista.List(idx, 17))
    C_Fone_Cont3.Value = SafeListVal(C_Lista.List(idx, 18))
    C_Func_Cont3.Value = SafeListVal(C_Lista.List(idx, 19))
    C_InfoAD.Value = SafeListVal(C_Lista.List(idx, 20))
...
On Error GoTo erro_carregamento:
    ...
    Call LimparCamposCadastroEntidade
...
Private Sub LimparCamposCadastroEntidade()
    C_CNPJ.Value = Empty
    C_Entidade.Value = Empty
    C_Tel_Fixo.Value = Empty
    C_Tel_Cel.Value = Empty
    C_Email.Value = Empty
    C_Endereco.Value = Empty
    C_Bairro.Value = Empty
    C_Municipio.Value = Empty
    C_CEP.Value = Empty
    C_UF.Value = Empty
    C_Contato1.Value = Empty
    C_Fone_Cont1.Value = Empty
    C_Func_Cont1.Value = Empty
    C_Contato2.Value = Empty
    C_Fone_Cont2.Value = Empty
    C_Func_Cont2.Value = Empty
    C_Contato3.Value = Empty
    C_Fone_Cont3.Value = Empty
    C_Func_Cont3.Value = Empty
    C_InfoAD.Value = Empty
End Sub
...
Private Sub Btn_Rel_OS_Empresa_Click()
    ...
    Call UI_DescartarFormVisivel("Rel_OSEmpresa")
...
Call UI_DescartarFormVisivel("Rel_Emp_Serv")
...
Private Function AbrirURLExterna(ByVal url As String) As Boolean
    ...
    If InStr(1, Application.OperatingSystem, "Mac", vbTextCompare) > 0 Then
        ...
        End If
        Err.Clear
    ElseIf InStr(1, Application.OperatingSystem, "Windows", vbTextCompare) > 0 Then
...
Private Sub TextBox19_Change()   ' Emite Solicitacao de Servico (OS)
    If mInicializando Then Exit Sub
    On Error Resume Next
    Call Preencher_FiltrarPorBoxEstatico("os", CStr(TextBox19.Text))
    On Error GoTo 0
End Sub
...
Private Sub TextBox20_Change()   ' Avaliacao / Encerramento
    If mInicializando Then Exit Sub
    On Error Resume Next
    Call Preencher_FiltrarPorBoxEstatico("aval", CStr(TextBox20.Text))
    On Error GoTo 0
End Sub
