#!/usr/bin/env bash
# =============================================================================
# scripts/hbn-guards/hbn-guards-runner.sh  (P2-C2 — runner project-mode)
...
# assert-knowledge-index AGORA e bloqueante (INDEX reconciliado em P2-C2).
# Guards internos do genoma NAO rodam aqui (§2). Fail-fast; sem bypass mascarado.
# =============================================================================
set -euo pipefail
...
if [[ $OVERALL -eq 0 ]]; then
  echo "${C_BOLD}[hbn-guards] Subset project-mode verde (integridade + 7 guards).${C_END}" >&2
else
