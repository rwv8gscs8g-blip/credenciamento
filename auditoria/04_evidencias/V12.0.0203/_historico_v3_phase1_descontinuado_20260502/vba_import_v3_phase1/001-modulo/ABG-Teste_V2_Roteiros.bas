...
Public Sub TV2_RunRodizioStrikesEndToEnd(Optional ByVal visual As Boolean = False, Optional ByVal silencioso As Boolean = False)
    ...
    For i = 1 To 3
        ...
        TV2_LogAssert "STRIKES_E2E", "CS_E2E_E_VOLTA_" & i, "AUTO", _
            "Etapa E volta " & i & ": EMP2 acumula " & i & " strike(s)", _
            "STRIKES=" & i, _
            "STATUS=" & emp.STATUS_GLOBAL & "; STRIKES=" & contStrikes2, _
            "EMP2 vira o foco quando EMP1 esta fora", _
            contStrikes2 = i
    Next i
    emp = LerEmpresa("002", linhaEmp)
    TV2_LogAssert "STRIKES_E2E", "CS_E2E_E_FINAL_SUSP", "AUTO", _
        "Apos 3 strikes, EMP2 deve estar SUSPENSA", _
        "STATUS=SUSPENSA_GLOBAL", _
        "STATUS=" & emp.STATUS_GLOBAL, _
        "Confirma regra simetrica: ambas empresas seguem mesma logica", _
...
Private Sub TV2_E2E_PrepararCenario()
    ...
    If Not jaExiste Then
        ...
        End If
        linha = TV2_E2E_NextDataRow(SHEET_ATIVIDADES)
    ...
    If Not jaExiste Then
        ...
        End If
        linha = TV2_E2E_NextDataRow(SHEET_CAD_SERV)
...
Private Function TV2_E2E_AtenderProximaEmpresa(ByVal notaE1 As Integer, _
    ...
    Select Case sel.Empresa.EMP_ID
        Case "001": notaUniforme = notaE1
        Case "002": notaUniforme = notaE2
        Case "003": notaUniforme = notaE3
    ...
    If Not resPre.Sucesso Then
        TV2_LogInfo "STRIKES_E2E", "E2E_PREOS_FAIL", _
            "EmitirPreOS falhou para EMP=" & sel.Empresa.EMP_ID & " COD=" & TV2_E2E_ATIV_ID & "|" & TV2_E2E_SERV_ID, _
            "Erro: " & resPre.mensagem
        TV2_E2E_AtenderProximaEmpresa = 0
        Exit Function
    End If
    resOs = EmitirOS(resPre.IdGerado, Date + 7, "E2E-" & sel.Empresa.EMP_ID)
    If Not resOs.Sucesso Then
        TV2_LogInfo "STRIKES_E2E", "E2E_OS_FAIL", _
            "EmitirOS falhou PREOS=" & resPre.IdGerado & " EMP=" & sel.Empresa.EMP_ID, _
            "Erro: " & resOs.mensagem
        TV2_E2E_AtenderProximaEmpresa = 0
        Exit Function
    End If
    ...
    If Not resAval.Sucesso Then
        TV2_LogInfo "STRIKES_E2E", "E2E_AVAL_FAIL", _
            "AvaliarOS falhou OS=" & resOs.IdGerado & " EMP=" & sel.Empresa.EMP_ID & " NOTA=" & notaUniforme, _
...
' Helper local para a suite e2e: proxima linha de dados em uma aba.
' Substitui o TV2_NextDataRow (Private em Teste_V2_Engine, inacessivel
' aqui). Usa UltimaLinhaAba (Public em Util_Planilha).
Private Function TV2_E2E_NextDataRow(ByVal nomeAba As String) As Long
    Dim ultima As Long
    ultima = UltimaLinhaAba(nomeAba)
    If ultima < LINHA_DADOS Then
        TV2_E2E_NextDataRow = LINHA_DADOS
    Else
        TV2_E2E_NextDataRow = ultima + 1
    End If
End Function
