...
' ============================================================
' Hotfix Onda 9 (V12.0.0429): early-binding com tipos MSForms concretos.
'
' Bug: declaracao com `frm As Object` e retorno `As Object` quebrava
' compilacao no caller que tem `WithEvents mTxtBuscaTopo As MSForms.TextBox`.
' WithEvents exige tipo conhecido em compile-time; cadeia de cast
' Object -> MSForms.TextBox -> WithEvents quebrava.
'
' Fix: retorno tipado como MSForms.TextBox; iteracao por indice (For i)
' em vez de For Each (mais robusto cross-platform).
Public Function UtilFiltro_LocalizarTextBoxFiltro(ByVal frm As Object) As MSForms.TextBox
    On Error Resume Next
    Dim canonico As MSForms.TextBox
    Set canonico = frm.Controls("txtFiltro")
    Err.Clear
    If Not canonico Is Nothing Then
