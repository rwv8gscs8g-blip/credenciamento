...
Private Function CorrigirMojibakeBasico(ByVal s As String) As String
    ...
    t = Replace(t, "Ã´", "ô")
    t = Replace(t, "Ãµ", "õ")
    ' Hotfix Onda 9 (V12.0.0429): smart quotes via ChrW (sintaxe valida em VBA
    ' + imune a normalizacao de smart quotes do publicar_vba_import_v2.py).
    ' Original tinha "Ã" + smart-quote literal, que apos normalizacao virou
    ' "Ã" + aspa-reta gerando string com numero impar de aspas (sintaxe
    ' invalida). ChrW(8220) = U+201C left double quote, ChrW(8221) = U+201D
    ' right double quote. Estes nao aparecem como literais no .bas, logo o
    ' publish nao os toca.
    t = Replace(t, "Ã" & ChrW(8220), "Ó")
    t = Replace(t, "Ã" & ChrW(8221), "Ô")
    t = Replace(t, "Ã•", "Õ")
    t = Replace(t, "Ãº", "ú")
    t = Replace(t, "Ãš", "Ú")
