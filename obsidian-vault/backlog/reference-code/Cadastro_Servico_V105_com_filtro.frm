...
If descServ = "" Then
    MsgBox "Informe a descri" & ChrW$(231) & ChrW$(227) & "o do servi" & ChrW$(231) & "o.", _
           vbExclamation, "Cadastro de Servico"
    Exit Sub
End If
...
If ativId = "" Then
    ...
    Else
        CNAE = Trim(InputBox("Atividade ainda nao cadastrada." & vbCrLf & _
                             "Informe o CNAE para criar a atividade '" & ativDesc & "':", _
                             "Novo CNAE / Atividade"))
        If CNAE = "" Then
            MsgBox "Cadastro cancelado: CNAE eh obrigatorio para nova atividade.", vbExclamation, "Cadastro de Servico"
            Exit Sub
        End If

        If Not Util_PrepararAbaParaEscrita(wsAtiv, estavaProtAtiv, senhaProtAtiv) Then
            MsgBox "Nao foi possivel cadastrar nova atividade: aba ATIVIDADES protegida.", vbCritical, "Cadastro de Servico"
            Exit Sub
        End If
...
If ServicoJaExiste(wsServ, ativId, descServ) Then
    If ativDesc = "" Then ativDesc = "(descricao da atividade nao informada)"
    MsgBox "Nao foi possivel cadastrar: este servico ja existe para a atividade selecionada." & vbCrLf & _
           "Atividade: " & ativId & " - " & ativDesc & vbCrLf & _
           "Servico: " & descServ & vbCrLf & _
           "Se necessario, atualize apenas o valor na tela principal.", _
           vbExclamation, "Cadastro de Servico"
    Exit Sub
End If
...
If Not Util_PrepararAbaParaEscrita(wsServ, estavaProtServ, senhaProtServ) Then
    MsgBox "Nao foi possivel cadastrar servico: aba CAD_SERV protegida.", vbCritical, "Cadastro de Servico"
    Exit Sub
End If
...
If Not Util_SalvarWorkbookSeguro(msgSave) Then
    MsgBox "Servico cadastrado, mas houve falha ao salvar o arquivo automaticamente." & vbCrLf & _
...
End If
    ...
    MsgBox msgCriacaoAtiv & "Servi" & ChrW$(231) & "o cadastrado com sucesso." & vbCrLf & _
       ...
       vbInformation, "Cadastro de Servico"
Unload Me
Exit Sub
...
On Error GoTo 0
MsgBox "Erro ao cadastrar servi" & ChrW$(231) & "o: " & Err.Description, vbCritical, "Cadastro de Servico"
End Sub
...
Call PreenchimentoListaAtividade(Trim$(CStr(S_Atividade.Value)))
...
On Error GoTo fim
If mIgnorarFiltro Then Exit Sub
Call PreenchimentoListaAtividade(Trim$(CStr(mTxtBuscaTopo.Text)))
fim:
End Sub
